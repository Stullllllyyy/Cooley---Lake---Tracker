...
What Is Huginn?
Huginn is a whitetail deer hunting intelligence web app built for a specific ~100-acre property near Suring, Wisconsin (Cooley Lake). It enables hunters to log trail camera sightings and field observations, track named individual bucks, analyze movement patterns, and get AI-powered hunt recommendations. Built as a single-file vanilla HTML/JS/CSS app with a Supabase backend.
The long-term vision is a multi-property, multi-user SaaS platform under the Huginn Advisory Group brand — "Know More. Hunt Smarter." Cooley Lake is Property #1 and the personal dogfood instance for all feature development.
Tech Stack
...
/ (root)   /api     claude.js         ← Vercel serverless proxy for Anthropic API   /public     index.html        ← Entire application (HTML + CSS + JS)     HuginnLogo.svg     HuginnFavicon.svg     HuginnRaven.png     map.jpg   CLAUDE.md           ← This file   PLANNING.md         ← Roadmap and product decisions   TASKS.md            ← Session tasks and backlog
Brand
...
Bug Notes Camera pin visual drift from movement lines Pins slightly offset from line endpoints at all zoom levels. Be885d1 is last stable commit before failed fix attempts. Approach carefully — pixel offset vs coordinate offset. Unknown Bucks — Marsh Buck appearing in unknown bucket buck_name staying null after AI suggestion not confirmed/saved
Session Handoff
